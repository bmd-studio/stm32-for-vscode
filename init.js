...
async function init(workspacePath) {
  // return listFiles();
  return extractFileTypes(workspacePath).then((output) => {
    ...
    extractMakeFileInfo(output.makefile).then((makefileInfo) => {
      ...
      updateBuildAndDebugFiles(makefileInfo.target, makefileInfo.targetMCU, workspacePath).then((output) => {
...
*/
async function updateBuildAndDebugFiles(firmwareName, targetName, workspacePath) {
  ...
  if (vscodeDirErr) {
    ...
    return new Promise((resolve, reject) => { reject(vscodeDirErr); });
  }
  ...
  const tasksProm = getTextFile(`${workspacePath}/.vscode/tasks.json`).then((tasksString) => {
    const buildTask = getConfigFiles('build', firmwareName, targetName, workspacePath);
    ...
    if (hasBuildTask && hasLoadTask) { return new Promise((resolve, reject) => { resolve(); }); }
    if (!hasBuildTask) {
...
function checkForRequirements(outputFunc) {
